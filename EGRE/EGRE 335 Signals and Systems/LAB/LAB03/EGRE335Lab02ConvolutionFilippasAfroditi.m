...
input = load('-mat', 'C:\Users\echo4\Class\EGRE\EGRE 335 Signals and Systems\LAB\LAB02\data01.mat');data = input.S1_r;
...
% <}. 
%% Enter your code here - keep to naming convention in comment:
stname = 'Downsampled every two points';
% sigDS2 = data();
...
%% N = 5, 10, 20, 200, 500
%% Enter your code here (keep to the naming convention in the comment):
% dataDS5 = 
% dataDS10 = 

%% Plot each case on same plot as the previously reduced data set; set the limits to 
...
%% (keep to the naming convention in the comment)
% {movemean}
% dataMM5 = 
...
original = fftshift(abs(fft(data)));
...
filtert = [enter fft command here];
figure('Name','fft')
yyaxis left;
plot(waxis2,original);
hold on;
yyaxis right;
plot(waxisf,filtert,'- r');

...
hold on
%plot(waxis2,abs(x6),'- .');
plot(waxis2,abs(real(altx6)),'- .');
plot(t,abs(imag(altx6)),'- .');
...
myfun = zeros(size(tf));
myfun(1) = 1;
filter80kHz = myfun-sinc(2*80E3*pi*tf);
filter50kHz = myfun-sinc(2*50E3*pi*tf);
filter30kHz = myfun-sinc(2*30E3*pi*tf);
filter20kHz = myfun-sinc(2*20E3*pi*tf);
...
legend('80kHz Filter','50kHz Filter','30kHz Filter','20kHz Filter')
...
filtered = conv(data,filter80kHz,'same');
ns = length(filtered);
...
filtert = abs(fftshift(fft(filtered)));
figure('Name','fft')
%yyaxis left;
plot(waxis2,abs(altx6));
hold on;
%yyaxis right;
plot(waxisf,filtert,'- r');
xlabel('Frequency f [Hz]');
ylabel ('|FFT| [V]');
title('Original FFT vs Filtered FFT')
legend('Original FFT', 'Filtered FFT')
